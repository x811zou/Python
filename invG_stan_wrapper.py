...
def runVariant(model,fields,input_file,tmp_output_file,stan_output_file,init_file):
    if(len(fields)>=5):
...
out2 = out_path+"model_med/"+str(outfix)+".pickle"
out3 = out_path+"model_prob/"+str(outfix)+".pickle"
out4 = out_path+"model_prob2/"+str(outfix)+".pickle"
#out4 = out_path+"baseline_theta/"+str(outfix)+"_s-"+str(sigma)+".pickle"

import os
if (os.path.isfile(out4)) and (os.path.isfile(out2)) and (os.path.isfile(out3)):
   print ("Already Processed"+str(outfix))
   os._exit(0)


model = "/data/reddylab/scarlett/1000G/software/cmdstan/examples/"+str(model)+"/"+str(model)
tmpFile= "tmp_output."+sys.argv[2]+".txt"
initFile = "initialization_stan."+sys.argv[2]+".txt"
outFile= "stan_output."+sys.argv[2]+".txt"
...
with open(input_file,"rt") as IN:
    for line in IN:
        ...
        med,prob,prob2=runVariant(model,fields,input_file,tmp_output_file,stan_output_file,init_file)
        if med is not None:
