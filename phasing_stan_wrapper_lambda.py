...
def runVariant(model,fields,input_file,tmp_output_file,stan_output_file,init_file,sigma,Lambda):
    if(len(fields)>=5):
    ...
    #/data/reddylab/scarlett/1000G/software/cmdstan/examples/ase/ase sample data file=/data/reddylab/scarlett/1000G/software/cmdstan/examples/ase/tmp_output.txt init=/data/reddylab/scarlett/1000G/software/cmdstan/examples/ase/initialization_stan.txt output file=/data/reddylab/scarlett/1000G/software/cmdstan/examples/ase/output_theta.txt      
        ...
        max_prob1 = getMaxProb(thetas,Lambda)
...
model=sys.argv[4]
#in_path="/data/reddylab/scarlett/1000G/data/ASE_simulation/simulated_data/" #+sys.argv[3]+"/"
in_path=sys.argv[5]
#out_path="/data/reddylab/scarlett/1000G/software/cmdstan/examples/"+str(model)+"/new_simulation/output_pkl/" #+sys.argv[3]+"/"
out_path=sys.argv[6]
inFile=sys.argv[1]
...
out_path_1 = out_path+"model_med/"
if not os.path.exists(out_path_1):
    os.makedirs(out_path_1)
out_path_2 = out_path+"model_prob/"
if not os.path.exists(out_path_2):
    os.makedirs(out_path_2)
#out_path_3 = out_path+"/model_prob2/"
#if not os.path.exists(out_path_3):
#    os.makedirs(out_path_3)

import os.path
#out1 = out_path+"model_theta/"+str(outfix)+"_s-"+str(sigma)+".pickle"
out1 = out_path_1 + str(outfix)+"_s-"+str(sigma)+".pickle"
out2 = out_path_2 + str(outfix)+"_s-"+str(sigma)+".pickle"
#out3 = out_path_3 + str(outfix)+"_s-"+str(sigma)+".pickle"
...
model = "/data/reddylab/scarlett/1000G/software/cmdstan/examples/"+str(model)+"/"+str(model)
...
with open(input_file,"rt") as IN:
    for line in IN:
        ...
        med,prob=runVariant(model,fields,input_file,tmp_output_file,stan_output_file,init_file,sigma,Lambda)
        if med is not None:
...
pickle.dump(model_theta_med,open(out1,'wb'))
pickle.dump(model_med_prob,open(out2,'wb'))
#pickle.dump(model_med_prob2,open(out3,'wb'))
