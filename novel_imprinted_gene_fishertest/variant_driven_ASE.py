...
"""
python variant_driven_ASE.py ENSG00000177879 AP3S1
...
def read_and_process_file(cutoff_totalcount: int, file_path: str, geneID: str, is_counts: bool) -> pd.DataFrame:
    ...
    if is_counts:
        ...
        df["average_count"] = df["total_count"]/df["n_hets"]
        df = df[df["average_count"] >= int(cutoff_totalcount)]
        df = df[["Gene.stable.ID", "total_count", "n_hets", "average_count", "pvals_corrected"]][df["Gene.stable.ID"] == geneID]
    return df
...
def collect_info_one_gene(cutoff_totalcount: int, data_dir: str, geneID: str, gene_name: str, outfile = False):
    ...
    for sub_dir in subfolders:
        ...
        if os.path.isfile(counts_file) and os.path.isfile(qb_file):
            ...
            if not df_counts.empty and not df_qb.empty:
                ...
                df_counts = df_counts[["geneID","Gene.stable.ID", "Gene.name","chr","pos","rsid","ref","refCount","alt","altCount"]]
            else:
                continue
...
def collect_info_common_SNPs(data, outfile = False):
...
def get_variant_combination(data, data_snp, data_snp_list, cutoff_nind, n_total_ind):
    print(
        "geneID\tgeneName\tchr\tSNPs_combination\ttotal_n\tn_SNP\tn_noSNP\tn_individual\tpercentage_individual\tfishexactP\tASE_SNP\tnoASE_SNP\tASE_noSNP\tnoASE_noSNP\n"
    )
...
def get_variant(data, data_snp, data_snp_list, cutoff_nind, n_total):
    print(
        "geneID\tgeneName\tchr\tSNP\tvariant\ttotal_n\tn_SNP\tn_noSNP\tn_individual\tpercent_individual\tfishexactP\tASE_SNP\tnoASE_SNP\tASE_noSNP\tnoASE_noSNP\n"
    )
...
def get_haplotype(data, data_snp, data_snp_list, cutoff_nind, n_total):
    print(
        "geneID\tgeneName\tchr\tpos\trsid\ttotal_individual\tn_individual\tpercent_individual\tfishexactP\tASE_REF\tASE_ALT\tnoASE_REF\tnoASE_ALT\n"
    )
    ...
    for pos in data_snp_list:
        if data_snp.to_dict()[pos] >= cutoff_nind:
            ...
            (
                ...
                noASE_REF,
                noASE_ALT,
            ) = calculate_2sidedfisherexact_haplotype(pos,data)
            gene_candidate = data["Gene.stable.ID"].iloc[0]
            gene_name = data["Gene.name"].iloc[0]
            info = [
                [
...
def calculate_2sidedfisherexact_haplotype(variant, data):
    data_filtered = data[data["pos"] == variant]
    ASE_REF = (
        data_filtered[data_filtered["ASE_gene"] == "ASE"]["compare_2allele"] == "REF"
    ).sum()
    ASE_ALT = (
        data_filtered[data_filtered["ASE_gene"] == "ASE"]["compare_2allele"] == "ALT"
    ).sum()
    noASE_REF = (
        data_filtered[data_filtered["ASE_gene"] == "no_ASE"]["compare_2allele"] == "REF"
    ).sum()
    noASE_ALT = (
        data_filtered[data_filtered["ASE_gene"] == "no_ASE"]["compare_2allele"] == "ALT"
    ).sum()
    #           ASE   no ASE\n",
    # REF>ALT    13      8\n",
    # REF<=ALT   14      12\n",
...
ENSG00000164308.12      ENSG00000164308 chr5    96231000        rs2549782       G       88      T       1       89      NA12829 ASE     REF
    ...
    if os.path.isfile(common_SNP_file):
        common_SNP = pd.read_csv(
            common_SNP_file, sep="\t", header=0
        )
    else:
        common_SNP = collect_info_one_gene(cutoff_totalcount, data_dir, gene_candidate, gene_name, common_SNP_file)
        ## (2) collect all SNP combination frequency information
    """
    pos     count
96254817        219
